...
const TABS: { id: TabId; label: string; icon: React.ReactNode; group: string; desc: string }[] = [
  ...
  { id: 'repurpose', label: 'Repurpose',   icon: <Repeat size={15} />,           group: 'Core',     desc: 'Take existing content and rewrite it for different channels and formats' },
...
function PipelineTab({
  ...
  // Post-generation: Chain + Carousel
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const t = generated.topic || topic || 'Content distribution';
                    const { result } = await api.chain(t, kb.target_audience, 5, 'Generate leads and build authority');
                    setChainResult(result);
                  } catch (e: any) { setError(e.message); }
                  finally { setChainLoading(false); }
                }}
              >
                {chainLoading ? <Spinner /> : <><Link2 size={14} /> Content Chain</>}
              </button>
              {generated.content?.['Blog'] && (
                <button
                  className="text-sm px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                  disabled={carouselLoading}
                  onClick={async () => {
                    setCarouselLoading(true);
                    try {
                      const blogContent = generated.content?.['Blog']?.content || '';
                      const { result } = await api.carousel(blogContent.slice(0, 200), 'LinkedIn', 6, kb.brand_voice || 'professional');
                ...
                >
                  {carouselLoading ? <Spinner /> : <><Layers size={14} /> Carousel from Blog</>}
                </button>
              )}
            </div>
          </div>
...
const MAEVEN_CONTENT = {
  ...
  'Twitter/X': {
    content: "100,000 stores. 12,000 shoppers surveyed.\n\n73% of carts abandoned — but not because of price.\n\nThe real reason: fear.\n\"Wrong size. Wrong look. Painful return.\"\n\nStores that removed risk instead of adding urgency?\n\n73% → 31% abandonment.\n\nTrust > pressure.",
    notes: 'Single tweet, 255 chars — stat contrast is the hook, fits without threading',
  },
  'Email': {
...
function DiscoverTab() {
  ...
  const [trendAudience, setTrendAudience] = useState('');
  ...
  const [dataChannels, setDataChannels] = useState(['LinkedIn', 'Twitter/X']);
  const [dataResult, setDataResult] = useState<any>(null);
  const [dataLoading, setDataLoading] = useState(false);
  const [dataError, setDataError] = useState<string | null>(null);

  const runTrends = async () => {
    setTrendsError(null); setTrendsLoading(true);
    try { const { result: r } = await api.trends(industry, trendAudience); setTrendsResult(r); }
    catch (e: any) { setTrendsError(e.message); }
    finally { setTrendsLoading(false); }
  };

  const runData = async () => {
    setDataError(null); setDataLoading(true);
    try { const { result: r } = await api.dataInsights(data, dataContext, dataChannels); setDataResult(r); }
  ...
  return (
    <div className="space-y-4">
      {/* Mode toggle */}
      <div className="card p-4">
        <p className="section-header mb-1">Discover — Find What to Write About</p>
        <p className="text-xs text-gray-500 mb-3">Don't know what to write? Two ways to find your next content topic.</p>
        <div className="flex gap-2">
          <button onClick={() => setMode('trends')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'trends' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
            <TrendingUp size={14} /> Trend Radar
          </button>
          <button onClick={() => setMode('data')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'data' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
            <BarChart2 size={14} /> Data → Story
          </button>
        </div>
      </div>
      ...
      {mode === 'trends' ? (
        <>
          <div className="card p-5 space-y-3">
            <p className="section-header">Trend Radar — What's trending in your industry?</p>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs text-gray-500 mb-1 block">Industry / Niche</label><input className="input w-full" placeholder="SaaS / Fashion / HealthTech" value={industry} onChange={e => setIndustry(e.target.value)} /></div>
              <div><label className="text-xs text-gray-500 mb-1 block">Target Audience (optional)</label><input className="input w-full" placeholder="E-commerce directors…" value={trendAudience} onChange={e => setTrendAudience(e.target.value)} /></div>
            </div>
            <button className="btn-primary flex items-center gap-2" onClick={runTrends} disabled={!industry || trendsLoading}>
              {trendsLoading ? <Spinner /> : <><TrendingUp size={14} /> Scan for Trends</>}
            </button>
          </div>
          {trendsError && <ErrorBox msg={trendsError} />}
          {trendsResult?.trends?.map((t: any, i: number) => (
          ...
          {trendsResult && <p className="text-xs text-gray-400 text-center">💡 Copy a trend and paste it into the Pipeline tab to generate full content.</p>}
        </>
      ) : (
        <>
          <div className="card p-5 space-y-3">
            <p className="section-header">Data → Story — Turn numbers into narratives</p>
            <textarea className="input w-full h-24 resize-none" placeholder={"Paste your data, stats, research findings…\ne.g. 'Our Q3 data: 73% of users who onboard in under 5 minutes retain for 6+ months.'"} value={data} onChange={e => setData(e.target.value)} />
            <input className="input w-full" placeholder="Brand context (optional)" value={dataContext} onChange={e => setDataContext(e.target.value)} />
            <div className="flex flex-wrap gap-2">
              {CHANNELS.map(ch => <button key={ch} onClick={() => setDataChannels(prev => prev.includes(ch) ? prev.filter(c => c !== ch) : [...prev, ch])}
                className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${dataChannels.includes(ch) ? 'border-brand-500 bg-brand-100 text-brand-600' : 'border-gray-300 text-gray-500'}`}>{ch}</button>)}
            </div>
            <button className="btn-primary flex items-center gap-2" onClick={runData} disabled={!data || dataLoading}>
              {dataLoading ? <Spinner /> : <><BarChart2 size={14} /> Convert to Content</>}
            </button>
          </div>
          ...
          {dataResult && (
            <div className="space-y-4">
              <div className="card p-4">
                ...
                <p className="text-sm text-gray-700">{dataResult.data_story}</p>
                <p className="text-xs text-gray-500 mt-2">Key stat: <span className="text-brand-600 font-medium">{dataResult.key_stat}</span></p>
              </div>
              {Object.entries(dataResult.content ?? {}).map(([ch, val]: any) => (
                <div key={ch}>
...
function ToolkitTab() {
  ...
  return (
    <div className="space-y-4">
      {/* Mode toggle */}
      <div className="card p-4">
        <p className="section-header mb-1">Toolkit — Standalone Analysis Tools</p>
        <p className="text-xs text-gray-500 mb-3">Three tools, each with its own input. No Pipeline needed.</p>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setMode('seo')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'seo' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
            <Search size={14} /> SEO Optimizer
          </button>
          <button onClick={() => setMode('voice')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'voice' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
            <Mic size={14} /> Brand Voice
          </button>
          <button onClick={() => setMode('score')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'score' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
            <Star size={14} /> Content Score
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-lg w-full mb-10">
        {[
          { icon: <Zap size={14} />, label: 'Pipeline', desc: 'LinkedIn, Email, Twitter, Blog — plus distribution chain' },
          { icon: <Repeat size={14} />, label: 'Repurpose', desc: 'Adapt existing content for new channels' },
          { icon: <TrendingUp size={14} />, label: 'Discover', desc: 'Trend Radar + Data → Story in one place' },
          { icon: <Search size={14} />, label: 'Toolkit', desc: 'SEO, Brand Voice cloning, Content Scoring' },
        ].map((item, i) => (
