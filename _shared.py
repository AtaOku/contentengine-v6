...
def call_claude(system: str, user: str, max_tokens: int = 2000, api_key: str | None = None) -> tuple[str, dict]:
    """Call Claude API. Returns (text, usage_dict).
    api_key from request body takes priority; env var is fallback."""
    key = api_key or os.environ.get("ANTHROPIC_API_KEY", "")
    if not key:
        raise ValueError("No API key provided. Enter your Anthropic API key to use this tool.")
